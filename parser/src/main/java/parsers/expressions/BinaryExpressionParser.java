...
public class BinaryExpressionParser implements ExpressionParser {
  ...
  private List<Token> removeUnnecessaryParentheses(List<Token> tokens) {
    ...
    while (tokens.size() > 2
        && tokens.get(0).tokenType() == TokenSyntaxType.OPEN_PARENTHESIS
        && tokens.get(tokens.size() - 1).tokenType() == TokenSyntaxType.CLOSE_PARENTHESIS) {
      ...
      for (int i = 0; i < tokens.size(); i++) {
        ...
        if (token.tokenType() == TokenSyntaxType.OPEN_PARENTHESIS) {
          level++;
        } else if (token.tokenType() == TokenSyntaxType.CLOSE_PARENTHESIS) {
          level--;
        }
  ...
  private boolean isOperator(Token token) {
    return token.tokenType() == TokenDataType.OPERAND;
  }
